...
export const Savatcha = () => {
  ...
  const [lengthItemPrice, setLengthItemPrice] = useState(0);

  const Minus = () => {
    setLengthItemPrice(lengthItemPrice - 1);
    if (lengthItemPrice > 0) {
      setLengthItemPrice(lengthItemPrice - 1);
    }
  };
  const DeleteData = (id) => {
          ...
          <Box display={"flex"} flexDirection={"column"}>
            {basket.map((item) => (
              <Box
                ...
                key={item.id}
                background={"#fff"}
                w={{ lg: "690px", xl: "932px", "2xl": "1000px" }}
                ...
                >
                  ...
                  <Box display={"flex"} flexDirection={"column"}>
                    <Box display={"flex"} gap={3}>
                      <Text color={"#282f3c"}fontSize={{ base: "13px", md: "18px" }}fontWeight={"800"}>{item.current_price_formatted}</Text>
                      <Heading display={{ base: "none", xl: "flex" }}p={"5px 8px"}color={"white"}borderRadius={"100px"}fontSize={{ base: "13px", md: "18px" }}background={"#DD1470"}>Toliq tolov{">"}</Heading>
                    </Box>
                    <Box display={"flex"} gap={"7px"}>
                      <Text pt={1.2}color={"#282f3c"}fontSize={{ base: "10px", md: "18px" }}fontWeight={"500"}_hover={{ color: "#DD1470" }}>{item.name}</Text>
                    </Box>
                    <Text pt={2}>Mahsulot kodi: 4688</Text>
                  </Box>
                </Box>
                <Box w={{ base: "100%", md: "auto" }}display={"flex"}gap={"30px"}pr={7}alignItems={"center"}
                >
                  <Box pl={{ base: "20px", md: "0px" }}display={"flex"}gap={7}
                  >
                    <DeleteIcon
              ...
              {data.map((item) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Image w={"24px"} h={"24px"} src={basketIcon} />
                    </Button>
                  </Box>
                </GridItem>
              ))}
